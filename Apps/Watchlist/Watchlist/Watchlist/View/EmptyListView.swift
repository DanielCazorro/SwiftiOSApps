...
struct EmptyListView: View {
    var body: some View {
        ContentUnavailableView {
            ...
            GroupBox {
                TabView {
                    PageTabView(icon: "1.circle", description: "Add some movies to the watchlist if you plan to watch the later")
                        .padding(.bottom, 36)
                    PageTabView(icon: "2.circle", description: "Later on you can also chooose a random film to watch")
                        .padding(.bottom, 36)
                }
